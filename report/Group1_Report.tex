...
0.908}, \textbf{ROC-AUC of 0.999}, \textbf{recall of 96.8\%}, and avoids \textbf{99.8\%}
of fraud loss while flagging only \textbf{1.6\%} of transactions.
...
\code{is\_night} flag captures this — night transactions carry a 1.74\% fraud rate versus
...
while the temporal \code{is\_night} flag lifts it far more — from 0.10\% to 1.74\%
...
\midrule
\code{is\_new\_device} & 0.101 & 0.270\\
\code{shipping\_billing\_mismatch} & 0.106 & 0.354\\
\code{is\_disposable\_email} & 0.116 & 0.380\\
\code{high\_risk\_country} & 0.115 & 0.312\\
\code{is\_night} & 0.100 & 1.745\\
\bottomrule
\end{tabular}
...
Browser and OS carry essentially no fraud signal (rates flat at $\approx$0.12–0.14\%), as
expected from uniform synthetic generation. Billing country does carry signal — the
high-risk countries RU/ID/NG/CN top the chart at 0.27–0.37\%
(Figure~\ref{fig:channels}). Finally, linear correlations with the target
(Figure~\ref{fig:corr}) are all individually weak ($|r|<0.09$): \code{amount},
\code{errorBalanceDest}, \code{isFlaggedFraud}, \code{orig\_drained}, and \code{is\_night}
...
AUC-PR (0.908) and F1 (0.399), while still catching 96.8\% of fraud and avoiding 99.8\% of
loss. Logistic Regression confirms the EDA's hint — a linear model cannot separate this data
(test AUC-PR 0.586) and compensates with an untenable flag rate. Random Forest is
...
\midrule
\textbf{XGBoost} & \textbf{0.908} & 0.999 & 0.251 & 0.968 & \textbf{0.399} & 99.82\%\\
Random Forest & 0.812 & 0.970 & 0.086 & 0.946 & 0.158 & 99.57\%\\
Logistic Regression & 0.586 & 0.990 & 0.025 & 0.997 & 0.049 & 99.98\%\\
\bottomrule
\end{tabular}
...
in \code{realistic} yields strong, honest performance (0.908). This is the core empirical
...
\code{realistic} & no & 44 & \textbf{0.908} & 0.968 & 99.82\%\\
\code{synth} & no & 21 & 0.255 & 0.971 & 96.99\%\\
\code{dest}  & no & 16 & 0.010 & 0.997 & 99.59\%\\
\bottomrule
\end{tabular}
...
(44 features)} with a validation-selected threshold of \textbf{0.380}. Its full profile is
in Table~\ref{tab:deployed}. The validation$\to$test AUC-PR change (0.816$\to$0.908) is
...
\midrule
AUC-PR & 0.816 & 0.908\\
ROC-AUC & 0.9992 & 0.9988\\
Operating threshold & \multicolumn{2}{c}{0.380 (selected on validation)}\\
Precision & 0.045 & 0.251\\
Recall & 0.984 & 0.968\\
F1 & 0.086 & 0.399\\
Flagged rate & 1.29\% & 1.62\%\\
Loss avoided & 99.97\% & 99.82\%\\
\bottomrule
\end{tabular}
\caption{Deployed model: XGBoost / \code{realistic} / 44 features / threshold 0.380.}
\label{tab:deployed}
\end{table}

\subsection{Interpreting the operating point}

At the chosen threshold on the test window the model flags 1.62\% of all transactions,
catches 96.8\% of frauds, and intercepts 99.8\% of the fraudulent money. Precision is 25\%
by design (Section~\ref{sec:cost}): three of every four flags are false alarms sent to
...
(0.380) and a high-confidence auto-block threshold, enabling an \emph{allow / review / block}
...
as the result would be misleading. The deployable 0.908 is the number the business can
...
\begin{keybox}
\textbf{Demo vs.\ reference build.} The results above are the reference \textbf{full-data
(6.36M-row)} build. For a free-tier live demo (Chapter~\ref{ch:m6}) the same pipeline is
rebuilt on a committed \textbf{100k-row synthetic stand-in}
(\code{data/processed/transactions\_context.parquet}, $\approx$9\,MB) so the hosted app boots
in seconds and can even retrain in-browser; on that small, easily separable sample the
\code{realistic} models score near-perfectly, which is expected and is \emph{not} the headline
result. The full-data 0.908 AUC-PR remains the reference figure of merit.
\end{keybox}
...
runtime artefacts (the ensemble bundle, the 9\,MB synthetic stand-in, and the preview CSV,
kept in git via \code{.gitignore} exceptions), point Streamlit Cloud at
...
\begin{itemize}[nosep,leftmargin=1.4em]
  ...
  \item The hosted demo runs on a 100k synthetic stand-in and can retrain on fresh seeds, so
        its live numbers are optimistic and volatile; the full-data 0.908 AUC-PR is the
        reference result.
\end{itemize}
...
XGBoost model reaches a test AUC-PR of 0.908, recall of 96.8\%, and avoids 99.8\% of fraud
loss while flagging only 1.6\% of transactions — an operating point chosen not by accuracy
...
\bottomrule
\endfoot
Logistic Reg. & base & yes & 18 & 0.533 & 0.774 & 0.119 & 0.993 & 0.212 & 98.85\%\\
Logistic Reg. & dest & no & 16 & 0.001 & 0.011 & 0.004 & 0.998 & 0.008 & 99.94\%\\
Logistic Reg. & synth & no & 21 & 0.064 & 0.217 & 0.010 & 1.000 & 0.020 & 100.0\%\\
Logistic Reg. & realistic & no & 44 & 0.265 & 0.586 & 0.025 & 0.997 & 0.049 & 99.98\%\\
Logistic Reg. & all & yes & 50 & 0.612 & 0.853 & 0.114 & 0.989 & 0.205 & 98.85\%\\
\midrule
Random Forest & base & yes & 18 & 1.000 & 1.000 & 1.000 & 0.9995 & 0.9998 & 99.99\%\\
Random Forest & dest & no & 16 & 0.001 & 0.006 & 0.006 & 0.804 & 0.011 & 83.50\%\\
Random Forest & synth & no & 21 & 0.295 & 0.131 & 0.030 & 0.734 & 0.057 & 73.88\%\\
Random Forest & realistic & no & 44 & 0.748 & 0.812 & 0.086 & 0.946 & 0.158 & 99.57\%\\
Random Forest & all & yes & 50 & 1.000 & 1.000 & 1.000 & 0.9995 & 0.9998 & 99.99\%\\
\midrule
XGBoost & base & yes & 18 & 1.000 & 1.000 & 1.000 & 0.9995 & 0.9998 & 99.99\%\\
XGBoost & dest & no & 16 & 0.001 & 0.010 & 0.004 & 0.997 & 0.008 & 99.59\%\\
XGBoost & synth & no & 21 & 0.271 & 0.255 & 0.011 & 0.971 & 0.022 & 96.99\%\\
\textbf{XGBoost} & \textbf{realistic} & \textbf{no} & \textbf{44} & \textbf{0.816} & \textbf{0.908} & \textbf{0.251} & \textbf{0.968} & \textbf{0.399} & \textbf{99.82\%}\\
